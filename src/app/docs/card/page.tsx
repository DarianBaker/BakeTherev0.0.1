...
export function Card({ theme, className, children, ...props }) {
  return (
    <div {...(theme ? { "data-bt-theme": theme } : {})}
      className={cn("rounded-[var(--bt-radius-lg)] border border-[var(--bt-border)] bg-[var(--bt-bg-surface)] shadow-[var(--bt-shadow-sm)]", className)}
      {...props}>
      {children}
    </div>
  );
